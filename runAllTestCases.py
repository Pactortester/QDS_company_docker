...
logger = Log()
# 配置收发件人
recvaddress = ['zhangwenfeng@quandashi.com']
# 163的用户名和密码
sendaddr_name = 'zhangwenfeng@quandashi.com'
sendaddr_pswd = 'Qq34021500'
...
class SendMail:
    ...
    def send(self):
        ...
        try:
            smtp = smtplib.SMTP('smtp.quandashi.com', port=587)
            smtp.ehlo()
            smtp.starttls()
            smtp.ehlo()
...
if __name__ == "__main__":
    ...
    with open(path, 'wb+') as file:
        HTMLTestRunner(stream=file, verbosity=2, title="权大师测试报告", description="Environment ：docker centos7 headless Chrome", tester="lijiawei").run(suite)
    # send_mail(path)
    sendMail = SendMail()
    sendMail.send()
